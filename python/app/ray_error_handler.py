...
def handle_ray_worker_error(
    error: Exception, worker_type: str = "unknown"
) -> Exception:
    ...
    if any(
        keyword in error_str for keyword in ["worker", "crash", "died", "connection"]
    ):
...
def ray_worker_with_error_handling(worker_type: str, timeout: Optional[int] = None):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            logger.info(
                f"Starting {worker_type} worker - Memory: {initial_memory:.1f}MB"
            )
            ...
            try:
                ...
                logger.info(
                    f"Completed {worker_type} worker - Duration: {duration:.2f}s, "
                ...
                logger.error(
                    f"Failed {worker_type} worker - Duration: {duration:.2f}s, "
